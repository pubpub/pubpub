/**
 * A utility to help keep things in a user-defined order in the database. Imagine that we want to
 * keep a user-ordered list of items somewhere on PubPub -- this shouldn't be hard because we often
 * do, in places like the collection or attribution editor. While it's easy to store this
 * user-defined order in a `rank` column in the database, re-ordering elements is inefficient.
 * Imagine you have ranks [1, 2, ..., 100] in the database and you want to move the last item to the
 * beginning to get [100, 1, 2, ...]. This will require you to give the formerly 100th item rank 1
 * and update the 99 entries now behind it in the list, which will be quite slow!
 *
