function FirepadUserList(ref, place, userId, displayName, displayImage) {
  if (!(this instanceof FirepadUserList)) {
    return new FirepadUserList(ref, place, userId, displayName, displayImage);
  }
  ...
  this.displayName_ = displayName || 'Unknown';
  this.displayImage_ = displayImage;
  this.firebaseOn_(ref.root().child('.info/connected'), 'value', function(s) {
    if (s.val() === true && self.displayName_) {
      ...
      var imageRef = ref.child(self.userId_).child('image');
      imageRef.onDisconnect().remove();
      imageRef.set(self.displayImage_);
    }
  });
...
FirepadUserList.prototype.makeUserEntryForSelf_ = function() {
  ...
  // var nameInput = elt('div', null, {'class': 'firepad-userlist-name-input'} );
  ...
  var imageDiv = elt('img', null, {'src': this.displayImage_, 'class': 'firepad-userlist-image' });

  return elt('div', [ imageDiv, colorDiv, nameDiv ], {
...
FirepadUserList.prototype.makeUserEntriesForOthers_ = function() {
  ...
  //     console.log('addHoverClass');
  // }

  // function removeHoverClass(classname) {
  //   console.log('hi');
  // }
  ...
  function updateChild(userSnapshot, prevChildName) {
    ...
    var image = userSnapshot.child('image').val();
...
export default FirepadUserList;
