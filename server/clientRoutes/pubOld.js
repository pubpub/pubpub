...
app.get(
	[
		'/pubold/:slug',
		'/pubold/:slug/content/:sectionId',
		'/pubold/:slug/draft',
		'/pubold/:slug/draft/content/:sectionId',
		'/pubold/:slug/:mode',
		'/pubold/:slug/:mode/:subMode',
	],
	(req, res, next) => {
		...
		const isDraftRoute = req.path.indexOf(`/pubold/${req.params.slug}/draft`) === 0;
		...
		if (req.params.mode && acceptedModes.indexOf(req.params.mode) === -1) {
			return next();
		}

		return getInitialData(req)
			.then((initialData) => {
				return Promise.all([initialData, findPub(req, initialData, isDraftRoute)]);
			})
			...
			.then(([initialData, pubData, firebaseToken]) => {
				const isUnlistedDraft =
					!pubData.activeVersion.isPublic && pubData.draftPermissions === 'private';
