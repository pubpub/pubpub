...
app.get('/testLogin', function(req,res){
	...
	Journal.findOne({ $or:[ {'subdomain':referDomain.split('.')[0]}, {'customDomain':referDomain}]}, {'_id':1}).lean().exec(function(err, journal){
		if (journal) {
			return res.status(201).type('.html').send('<div><script type="text/javascript">var loginCookie = ''; try {loginCookie = "connect.sid="+document.cookie.split("connect.sid=")[1].split(";")[0]+";";}catch(err){} parent.postMessage(loginCookie, "' + req.get('referrer') + '");</script></div>');		
		}
