...
export const ExperimentsDinosaurPaper = React.createClass({
	...
	setWidth: function() {
		...
		// if (this.props.mode === 0) {
			this.setState({ 
				width: Math.min(element.offsetWidth, 767),
				margin: (element.offsetWidth - 767) / 2 + 20,
			});	
		// }
		...
		return (
			<div id={'article-container'}>
				...
				<div>
					...
					<ScatterChart width={this.state.width} height={this.state.width / 1.5} margin={{ top: 20, left: this.state.margin, right: 50, bottom: 20 }}>
				...
				{this.props.mode === 1 &&
					<div style={{ maxWidth: '700px', margin: '0 auto' }}>
						{this.state.offsets.map((item, index)=> {
							return <Slider key={`slider-${index}`} min={0} max={20} stepSize={0.1} value={item} onChange={this.sliderUpdate.bind(this, index)} />;
						})}
				...
				{this.props.mode === 2 &&
					<div style={{ maxWidth: '700px', margin: '0 auto' }}>
						<Textarea defaultValue={this.state.code} style={styles.codeInput} id={'code-textarea'}/>
