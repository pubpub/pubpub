...
import { apiFetch, generateHash } from 'utils';
...
class PubOptionsReview extends Component {
	...
	getStatusClassName(status) {
		if (status === 'unsubmitted' || status === 'closed') {
			return '';
		}
		if (status === 'submitted') {
		...
		return apiFetch('/api/pubs/reviews', {
			...
			body: JSON.stringify({
				reviewItem: {
					...
					status: this.state.selectedVersion ? 'submitted' : this.state.selectedStatus,
				},
	...
	render() {
		...
		if (currentStatus === 'unsubmitted' || currentStatus === 'closed') {
			instructions =
				'To begin a new review, select a version you would like to submit for consideration.';
		...
		if (currentStatus === 'submitted') {
			...
			buttonLanguage = (
				<span>
					...
					{!this.state.selectedStatus && !this.state.messageContent
		...
		if (currentStatus === 'accepted') {
			instructions =
				'Your version has been accepted! You may begin a new review on a new version below.';
			placeholder = 'Add a message to the review';
		...
		if (currentStatus === 'rejected') {
			instructions =
				'Your version has been rejected. You may begin a new review on a new version below.';
			placeholder = 'Add a message to the review';
			buttonLanguage = this.state.selectedVersion
				? 'Submit version for review'
				: 'Add message';
		...
		if (currentStatus === 'changes requested') {
			instructions =
				'Changes have been requested. You can make these changes, save them in a new version, and then select that version for review below.';
			placeholder = 'Add a message to the review';
			buttonLanguage = this.state.selectedVersion
		...
		return (
			<div className="pub-options-review-component">
				<div className="header-bar">
					<h1>Review</h1>
					<div>
						<span className={`bp3-tag ${this.getStatusClassName(currentStatus)}`}>
				...
				{!this.props.pubData.versions.length && (
					<div className="bp3-callout bp3-intent-warning">
						Reviews apply to specific versions. To begin a review you must first save a
						version of the working draft. You will then have options here to select the
						specific version you would like to submit for review.
					</div>
				)}
				...
				{(isAdmin || isManager) && !!this.props.pubData.versions.length && (
					<div>
							...
							<Select
								...
								itemRenderer={(item, { handleClick, modifiers }) => {
									return (
										...
										>
											Version:{' '}
											{dateFormat(item.createdAt, 'mmm dd, yyyy · h:MMTT')}
										</button>
									);
								}}
								onItemSelect={(item) => {
								...
								<Button
									text={
										this.state.selectedVersion
											? `Version: ${dateFormat(
													this.state.selectedVersion.createdAt,
													'mmm dd, yyyy · h:MMTT',
											  )}`
											: 'Select a Version'
									}
						...
						/>
						{currentStatus === 'submitted' && <div>Set Status:</div>}
								...
								<div className="statuses">
									<div className="bp3-button-group">
										<Button
											text="Submitted"
											className={
												!this.state.selectedStatus ? 'bp3-active' : ''
											}
											onClick={() => {
										...
										{isManager && (
											<Button
												text="Closed"
												className={
													this.state.selectedStatus === 'closed'
														? 'bp3-active'
														: ''
												}
												onClick={() => {
										...
										{isAdmin && (
											<Button
												...
												onClick={() => {
													this.setState({
														selectedStatus: 'changes requested',
													});
												}}
											/>
										)}
							...
							<div className="review-item" key={reviewItem.createdAt}>
								...
								<div className="item-header">
									...
									{reviewItem.status && (
										<div className="status-change">
											Status Changed to:{' '}
											<span
												className={`bp3-tag ${this.getStatusClassName(
													reviewItem.status,
												)}`}
											>
												{reviewItem.status}
								...
								{reviewItem.versionId && (
									<div className="bp3-callout">
										Submitted{' '}
										<a href={`/pub/${pubData.slug}?version=${version.id}`}>
											Version{' '}
